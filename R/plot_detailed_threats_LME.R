plot_detailed_threats_LME <- function(){
  ...
  mpa_pressures %>%
    ...
    ggplot(aes(reorder(LME_NAME,average_number_of_ports_LME), average_number_of_ports,
  ...
  mpa_pressures %>%
    ...
    ungroup() %>% 
    # dplyr::filter(SOVEREIGN1 != "Slovenia") %>%
    ggplot(aes(reorder(LME_NAME,average_number_of_offshore_LME), average_number_of_offshore,
               fill = match_status,group=match_status)) + 
    geom_col(stat="identity",position = "dodge",color="black",width = 0.7,lwd = 0.1) +
    scale_fill_manual(values = legend_match,name = "Cell type") +
    coord_flip() +
  ...
  mpa_pressures %>%
    ...
    ungroup() %>% 
    # dplyr::filter(SOVEREIGN1 != "Slovenia") %>%
    ggplot(aes(reorder(LME_NAME,average_number_of_shipping_LME), average_number_of_shipping,
               fill = match_status,group=match_status)) + 
    geom_col(stat="identity",position = "dodge",color="black",width = 0.7,lwd = 0.1) +
    scale_fill_manual(values = legend_match,name = "Cell type") +
    coord_flip() +
  ...
  mpa_pressures %>%
    ...
    ungroup() %>% 
    # dplyr::filter(SOVEREIGN1 != "Slovenia") %>%
    ggplot(aes(reorder(LME_NAME,average_number_of_fishing_LME), average_number_of_fishing,
               fill = match_status,group=match_status)) + 
    geom_col(stat="identity",position = "dodge",color="black",width = 0.7,lwd = 0.1) +
    scale_fill_manual(values = legend_match,name = "Cell type") +
    coord_flip() +
  ...
  mpa_pressures %>%
    #Group by match status
    group_by(LME_NAME,match_status) %>%
    mutate(average_number_of_dredge = mean(dredge_number)) %>%
    ungroup() %>% 
    #Get ecossystem average presure
    group_by(LME_NAME) %>%
    ...
    ungroup() %>% 
    # dplyr::filter(SOVEREIGN1 != "Slovenia") %>%
    ggplot(aes(reorder(LME_NAME,average_number_of_dredge_LME), average_number_of_dredge,
               fill = match_status,group=match_status)) + 
    geom_col(stat="identity",position = "dodge",color="black",width = 0.7,lwd = 0.1) +
    scale_fill_manual(values = legend_match,name = "Cell type") +
    coord_flip() +
  ...
  mpa_pressures %>%
    ...
    ungroup() %>% 
    # dplyr::filter(SOVEREIGN1 != "Slovenia") %>%
    ggplot(aes(reorder(LME_NAME,average_number_of_aggregate_LME), average_number_of_aggregate,
               fill = match_status,group=match_status)) + 
    geom_col(stat="identity",position = "dodge",color="black",width = 0.7,lwd = 0.1) +
    scale_fill_manual(values = legend_match,name = "Cell type") +
    coord_flip() +
  ...
  mpa_pressures %>%
    ...
    ungroup() %>% 
    # dplyr::filter(SOVEREIGN1 != "Slovenia") %>%
    ggplot(aes(reorder(LME_NAME,average_number_of_desalination_LME), average_number_of_desalination,
               fill = match_status,group=match_status)) + 
    geom_col(stat="identity",position = "dodge",color="black",width = 0.7,lwd = 0.1) +
    scale_fill_manual(values = legend_match,name = "Cell type") +
    coord_flip() +
  ...
  mpa_pressures %>%
    #Group by match status
    group_by(LME_NAME,match_status) %>%
    mutate(average_number_of_unmatched = mean(unmatched_number)) %>%
    ungroup() %>% 
    #Get ecossystem average presure
    group_by(LME_NAME) %>%
    ...
    ungroup() %>% 
    # dplyr::filter(SOVEREIGN1 != "Slovenia") %>%
    ggplot(aes(reorder(LME_NAME,average_number_of_unmatched_LME), average_number_of_unmatched,
               fill = match_status,group=match_status)) + 
    geom_col(stat="identity",position = "dodge",color="black",width = 0.7,lwd = 0.1) +
    scale_fill_manual(values = legend_match,name = "Cell type") +
    coord_flip() +
